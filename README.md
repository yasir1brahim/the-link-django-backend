...
```bash
mkvirtualenv the_link -p python3.11
...
Create a database named `the_link`.
...
```bash
celery -A the_link worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A the_link worker -l INFO -B --pool=solo
```
